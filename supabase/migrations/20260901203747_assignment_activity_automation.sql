...
begin
  if tg_table_name = 'assignments' then
    ...
    v_assignment_id := new.assignment_id; v_organization_id := new.organization_id;
    ...
    v_assignment_id := new.assignment_id; v_organization_id := new.organization_id; v_type := 'note_added'; v_metadata := jsonb_build_object('note_id', new.id);
  elsif tg_table_name = 'needs' then
    v_assignment_id := new.assignment_id; v_organization_id := new.organization_id; v_type := 'need_recorded'; v_metadata := jsonb_build_object('need_id', new.id, 'title', new.title, 'status', new.status);
  elsif tg_table_name = 'prayer_requests' then
    v_assignment_id := new.assignment_id; v_organization_id := new.organization_id; v_type := 'prayer_recorded'; v_metadata := jsonb_build_object('prayer_request_id', new.id, 'status', new.status);
  ...
  if v_assignment_id is not null and v_actor_id is not null then
    insert into public.assignment_activity (organization_id, assignment_id, actor_id, activity_type, metadata)
    values (v_organization_id, v_assignment_id, v_actor_id, v_type, coalesce(v_metadata, '{}'::jsonb));
  end if;
  return new;
end;
$$;
