...
import { Bell, CalendarDays, ChevronDown, Home, Menu, MessageCircle, ShieldCheck, Users, X } from 'lucide-react'
import { MissionMode } from '../features/mission/MissionMode'
import { TeamsWorkspace } from '../features/teams/TeamsWorkspace'
import './app.css'

type View = 'home' | 'mission' | 'teams' | 'events' | 'communication' | 'admin'
      ...
      <aside className={`rw-sidebar ${mobileOpen ? 'is-open' : ''}`} aria-label="Primary navigation">
        ...
        <nav>
          {navigation.map(({ id, label, icon: Icon }) => (
            <button key={id} className={`rw-nav-item ${view === id ? 'is-active' : ''}`} onClick={() => selectView(id)}>
              <Icon size={19} strokeWidth={2.1} /><span>{label}</span>
            </button>
          ))}
        </nav>
      ...
      <main className="rw-main">
        <header className="rw-topbar">
          ...
          <div className="rw-topbar-actions">
            <button className="rw-icon-button" aria-label="Notifications"><Bell size={19} /></button>
            <button className="rw-user-menu"><span className="rw-avatar">JB</span><span className="rw-user-copy"><strong>Jordan Bryan</strong><small>Organization Owner</small></span><ChevronDown size={16} /></button>
          </div>
        </header>
...
function Overview({ onNavigate }: { onNavigate: (view: View) => void }) {
  return <div className="rw-overview">
    <div className="rw-hero"><div><span className="rw-eyebrow">Ready to serve</span><h1>One place to organize the work that matters.</h1><p>Reachwell brings people, teams, events, communication, and field operations into one connected workspace.</p></div><button className="rw-primary-button" onClick={() => onNavigate('mission')}>Enter Mission Mode</button></div>
    <div className="rw-overview-grid">
      ...
      <button className="rw-feature-card" onClick={() => onNavigate('communication')}><MessageCircle size={22}/><strong>Keep teams connected</strong><span>Communication will stay connected to the organization, team, event, or assignment where it belongs.</span></button>
      <button className="rw-feature-card" onClick={() => onNavigate('events')}><CalendarDays size={22}/><strong>Run better events</strong><span>Plan, staff, check in, assign, serve, follow up, and measure impact.</span></button>
    </div>
  </div>
}

function ComingSoon({ title, description }: { title: string; description: string }) {
  return <div className="rw-empty-state"><span className="rw-eyebrow">Clean rebuild in progress</span><h1>{title}</h1><p>{description}</p></div>
}
