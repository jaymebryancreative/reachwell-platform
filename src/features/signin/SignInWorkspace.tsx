...
import { addPersonToEvent, listEventParticipants, listEvents, listPeople, updateAttendance, type EventParticipantRecord, type EventRecord, type PersonRecord } from '../../lib/reachwellApi'
...
export function SignInWorkspace({ eventId, onBack }: { eventId?: string; onBack: () => void }) {
  ...
  const add = async () => {
    ...
    try {
      ...
      if (!person) { person = await (await import('../../lib/reachwellApi')).createPerson({ organization_id: organizationId, first_name: newPerson.firstName.trim(), last_name: newPerson.lastName.trim(), preferred_name: null, email: null, phone: null, status: 'active' }) }
      const participant = await addPersonToEvent({ event_id: selectedEventId, person_id: person.id, role: newPerson.role.trim() || 'Volunteer' })
  ...
  return <div className="rw-signin"><div className="rw-signin-head"><div><button className="rw-back-button" onClick={onBack}><ArrowLeft size={17}/> Back to Events</button><span className="rw-eyebrow">Field-ready attendance</span><h1>Sign-In Mode</h1><p>Mark attendance for the selected event. Corrections are always allowed.</p></div><button className="rw-primary-button" onClick={() => setShowAdd(true)} disabled={!selectedEventId}><UserPlus size={18}/> Add new person</button></div>
    ...
    <div className="rw-signin-roster">{(loading || contextLoading) && <div className="rw-roster-empty">Loading roster…</div>}{!loading && !contextLoading && visible.map(participant => { const p = participant.person; const name = p ? (p.preferred_name || `${p.first_name} ${p.last_name}`) : 'Unknown person'; const status = participant.attendance_status; return <div className="rw-signin-row" key={participant.id}><div className="rw-member-avatar">{name.split(' ').map(x => x[0]).join('').slice(0,2)}</div><div className="rw-signin-person"><strong>{name}</strong><small>{participant.role}</small></div><div className="rw-attendance-actions"><button className={status === 'present' ? 'is-selected is-present' : ''} onClick={() => void mark(participant, 'present')}><Check size={17}/> Present</button><button className={status === 'late' ? 'is-selected is-late' : ''} onClick={() => void mark(participant, 'late')}><Clock3 size={17}/> Late</button><button className={status === 'absent' ? 'is-selected is-absent' : ''} onClick={() => void mark(participant, 'absent')}><X size={17}/> Not Present</button></div></div> })}{!loading && !contextLoading && selectedEventId && !visible.length && <div className="rw-roster-empty">No people are signed into this event yet. Use Add new person to build the roster.</div>}</div>
