...
import { ArrowLeft, Check, Clock3, UserPlus, Users, X, CheckCheck } from 'lucide-react'
import { addPersonToEvent, createPerson, listEvents, listPeople, type EventParticipantRecord, type EventRecord, type PersonRecord } from '../../lib/reachwellApi'
import { useReachWellContext } from '../../lib/reachwellContext'
import { supabase } from '../../lib/supabaseClient'
import './signin.css'

type SignInParticipant = EventParticipantRecord & { absence_reason: string | null }
const ABSENCE_REASONS = ['Sick', 'Family emergency', 'Work', 'Transportation', 'Out of town', 'Unknown', 'Other'] as const
...
export function SignInWorkspace({ eventId, onBack }: { eventId?: string; onBack: () => void }) {
  ...
  const loadParticipants = async (eventIdToLoad: string) => {
    const { data, error: queryError } = await supabase.from('event_participants').select('id, event_id, person_id, user_id, team_id, role, attendance_status, checked_in_at, absence_reason, person:people(id, first_name, last_name, preferred_name)').eq('event_id', eventIdToLoad).order('created_at')
    if (queryError) throw queryError
    return (data ?? []).map(row => ({ ...row, person: Array.isArray(row.person) ? (row.person[0] ?? null) : row.person })) as SignInParticipant[]
  }
  ...
  const load = async () => {
    ...
    try {
      ...
      if (activeId) { setSelectedEventId(activeId); setParticipants(await loadParticipants(activeId)) } else setParticipants([])
  ...
  useEffect(() => { if (selectedEventId && organizationId) void loadParticipants(selectedEventId).then(setParticipants).catch(err => setError(err instanceof Error ? err.message : 'Unable to load roster.')) }, [selectedEventId, organizationId])
  ...
  const mark = async (participant: SignInParticipant, status: 'present' | 'late' | 'absent', absenceReason: string | null = null) => {
    try {
      const checkedIn = status === 'present' || status === 'late' ? new Date().toISOString() : null
      const { data, error: updateError } = await supabase.from('event_participants').update({ attendance_status: status, checked_in_at: checkedIn, absence_reason: status === 'absent' ? absenceReason : null, updated_at: new Date().toISOString() }).eq('id', participant.id).select('id, event_id, person_id, user_id, team_id, role, attendance_status, checked_in_at, absence_reason, person:people(id, first_name, last_name, preferred_name)').single()
      if (updateError) throw updateError
      const updated = { ...data, person: Array.isArray(data.person) ? (data.person[0] ?? null) : data.person } as SignInParticipant
  ...
  const markAllPresent = async () => {
    const unmarked = participants.filter(p => !['present', 'late'].includes(p.attendance_status))
    if (!unmarked.length) return
    setSavingAll(true); setError(null)
    try {
      await Promise.all(unmarked.map(participant => supabase.from('event_participants').update({ attendance_status: 'present', checked_in_at: new Date().toISOString(), absence_reason: null, updated_at: new Date().toISOString() }).eq('id', participant.id)))
      setParticipants(await loadParticipants(selectedEventId));
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to mark the roster present.') } finally { setSavingAll(false) }
  }
  ...
  return <div className="rw-signin"><div className="rw-signin-head"><div><button className="rw-back-button" onClick={onBack}><ArrowLeft size={17}/> Back to Events</button><span className="rw-eyebrow">Field-ready attendance</span><h1>Sign-In Mode</h1><p>Mark attendance for the selected event. Corrections are always allowed.</p></div><div className="rw-signin-head-actions"><button className="rw-secondary-button" onClick={() => void markAllPresent()} disabled={!selectedEventId || savingAll || !participants.some(p => !['present', 'late'].includes(p.attendance_status))}><CheckCheck size={18}/> {savingAll ? 'Marking…' : 'Mark all present'}</button><button className="rw-primary-button" onClick={() => setShowAdd(true)} disabled={!selectedEventId}><UserPlus size={18}/> Add new person</button></div></div>
    ...
    <div className="rw-signin-roster">{(loading || contextLoading) && <div className="rw-roster-empty">Loading roster…</div>}{!loading && !contextLoading && visible.map(participant => { const p = participant.person; const name = p ? (p.preferred_name || `${p.first_name} ${p.last_name || ''}`).trim() : 'Unknown person'; const status = participant.attendance_status; return <div className="rw-signin-row" key={participant.id}><div className="rw-member-avatar">{name.split(' ').map(x => x[0]).join('').slice(0,2)}</div><div className="rw-signin-person"><strong>{name}</strong><small>{participant.role === 'participant' ? 'Volunteer' : participant.role}</small>{status === 'absent' && <select aria-label={`Absence reason for ${name}`} value={participant.absence_reason ?? ''} onChange={e => void mark(participant, 'absent', e.target.value || null)}><option value="">Reason for absence…</option>{ABSENCE_REASONS.map(reason => <option key={reason} value={reason}>{reason}</option>)}</select>}</div><div className="rw-attendance-actions"><button className={status === 'present' ? 'is-selected is-present' : ''} onClick={() => void mark(participant, 'present')}><Check size={17}/> Present</button><button className={status === 'late' ? 'is-selected is-late' : ''} onClick={() => void mark(participant, 'late')}><Clock3 size={17}/> Late</button><button className={status === 'absent' ? 'is-selected is-absent' : ''} onClick={() => void mark(participant, 'absent', participant.absence_reason)}><X size={17}/> Not Present</button></div></div> })}{!loading && !contextLoading && selectedEventId && !visible.length && <div className="rw-roster-empty">No people are signed into this event yet. Use Add new person to build the roster.</div>}</div>
