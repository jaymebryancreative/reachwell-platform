...
import { ArrowDownRight, ArrowUpRight, DollarSign, RefreshCw, ShieldCheck } from 'lucide-react'
...
export function MoneyTrailWorkspace() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data, error: loadError } = await supabase.from('financial_transactions').select('id,transaction_type,amount,currency,transaction_date,payee_or_source,description,status,reference_number,entered_by,approved_by,approved_at').eq('organization_id', organizationId).gte('transaction_date', start).order('transaction_date', { ascending: false }).limit(500)
      if (loadError) throw loadError
      const nextRows = (data ?? []) as Transaction[]
      setRows(nextRows)
      const actorIds = [...new Set(nextRows.flatMap(row => [row.entered_by, row.approved_by]).filter((id): id is string => Boolean(id)))]
      if (actorIds.length) {
  ...
  const summary = useMemo(() => rows.reduce((acc, row) => { const amount = amountFor(row); if (isIncome(row.transaction_type)) acc.income += amount; if (isExpense(row.transaction_type)) acc.expenses += amount; return acc }, { income: 0, expenses: 0 }), [rows])
