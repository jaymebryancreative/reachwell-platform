...
export function MissionMode() {
  ...
  const [backupType, setBackupType] = useState<'backup' | 'unsafe' | 'medical' | 'conflict' | 'other'>('backup')
  ...
  const complete = async () => {
    if (!current || !organizationId || !user) return
    try {
      const { data, error: updateError } = await supabase.from('assignments').update({ status: 'completed', completed_at: new Date().toISOString(), completed_by: user.id, completion_summary: summary || null, updated_at: new Date().toISOString() }).eq('id', current.id).eq('organization_id', organizationId).select().single()
      if (updateError) throw updateError
      setAssignments(items => items.filter(item => item.id !== current.id)); setCurrentId(null); setActiveAction(null); setMessage(data ? 'Assignment completed and recorded in history.' : 'Assignment completed.')
  ...
  const requestBackup = async () => {
    if (!current || !organizationId || !user) return
    try {
      const { error: insertError } = await supabase.from('assignment_safety_alerts').insert({ organization_id: organizationId, assignment_id: current.id, reporter_id: user.id, alert_type: backupType, message: backupMessage.trim() || null })
      if (insertError) throw insertError
      setBackupMessage(''); setBackupOpen(false); setActiveAction(null); setMessage('Back Up request sent to the organization team.')
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to send Back Up request.') }
  }
