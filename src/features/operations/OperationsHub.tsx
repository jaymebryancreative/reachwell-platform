...
function FinanceWorkspace({ rows, query, setQuery, loading }: { rows: LooseRow[]; query: string; setQuery: (value: string) => void; loading: boolean }) {
  const totals = useMemo<FinanceTotals>(() => rows.reduce<FinanceTotals>((acc, row) => {
    const amount = asNumber(row.amount); const type = asText(row.transaction_type).toLowerCase()
    if (type.includes('income') || type.includes('gift') || type.includes('donation') || type.includes('giving')) acc.income += amount
    else if (type.includes('expense') || type.includes('payment')) acc.expenses += amount
    return acc
  }, { income: 0, expenses: 0 }), [rows])
  const filtered = useMemo(() => rows.filter(row => JSON.stringify(row).toLowerCase().includes(query.toLowerCase())), [rows, query])
