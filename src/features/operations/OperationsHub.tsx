...
function Communication({ channels, channel, setChannel, message, setMessage, send, query, setQuery }: { channels: LooseRow[]; channel: string; setChannel: (value: string) => void; message: string; setMessage: (value: string) => void; send: () => void; query: string; setQuery: (value: string) => void }) { return <div className="ops-communication"><aside><div className="ops-channel-heading"><strong>Channels</strong><span>{channels.length}</span></div>{channels.map(item => <button key={asText(item.id)} className={channel === asText(item.id) ? 'selected' : ''} onClick={() => setChannel(asText(item.id))}><MessageCircle size={16} />{asText(item.name)}</button>)}{!channels.length && <p className="ops-muted">No channels yet.</p>}</aside><section><div className="ops-toolbar"><div className="ops-search"><Search size={16} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search visible messages" /></div></div><Messages channelId={channel} query={query} /><div className="ops-composer"><input value={message} onChange={event => setMessage(event.target.value)} placeholder="Write to the team…" onKeyDown={event => { if (event.key === 'Enter') send() }} /><button className="rw-primary-button" onClick={send} disabled={!message.trim()}>Send</button></div></section></div> }

function Messages({ channelId, query }: { channelId: string; query: string }) { const [items, setItems] = useState<LooseRow[]>([]); const [error, setError] = useState(''); useEffect(() => { if (!channelId) { setItems([]); return } const run = async () => { const { data, error: loadError } = await supabase.from('communication_messages').select('id,author_id,body,created_at').eq('channel_id', channelId).is('deleted_at', null).order('created_at', { ascending: true }); if (loadError) setError(loadError.message); else { setError(''); setItems(data ?? []) } }; void run() }, [channelId]); const filtered = items.filter(item => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())); return <div className="ops-message-list">{error && <div className="rw-context-alert">{error}</div>}{filtered.map(item => <div className="ops-message" key={asText(item.id)}><strong>{asText(item.author_id).slice(0, 8) || 'Member'}</strong><p>{asText(item.body)}</p><small>{new Date(asText(item.created_at)).toLocaleString()}</small></div>)}{!error && !filtered.length && <div className="ops-empty-inline">No matching messages.</div>}</div> }
