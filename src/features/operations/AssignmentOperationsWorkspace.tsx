...
type Assignment = {
  id: string; title: string; address_label: string | null; assignment_type: string; priority: number; sequence_number: number | null; status: string; event_id: string | null; person_id: string | null; household_id: string | null; assigned_team_id: string | null; assigned_user_id: string | null; started_at: string | null; completed_at: string | null; updated_at: string
}
...
export function AssignmentOperationsWorkspace() {
  ...
  const [assignments, setAssignments] = useState<Assignment[]>([]); const [teams, setTeams] = useState<Team[]>([]); const [members, setMembers] = useState<Member[]>([]); const [alerts, setAlerts] = useState<SafetyAlert[]>([])
  const [selectedId, setSelectedId] = useState(''); const [statusFilter, setStatusFilter] = useState('all'); const [priorityFilter, setPriorityFilter] = useState('all'); const [teamFilter, setTeamFilter] = useState('all'); const [query, setQuery] = useState('')
  ...
  const load = async () => {
    ...
    try {
      ...
      setAssignments(nextAssignments); setTeams((teamResult.data ?? []) as Team[]); setMembers((memberResult.data ?? []) as Member[]); setAlerts((alertResult.data ?? []) as SafetyAlert[])
      setSelectedId(current => current && nextAssignments.some(item => item.id === current) ? current : (nextAssignments[0]?.id ?? ''))
  ...
  useEffect(() => {
    if (!organizationId) return
    const channel = supabase.channel(`assignment-operations-${organizationId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'assignments', filter: `organization_id=eq.${organizationId}` }, () => void load()).on('postgres_changes', { event: '*', schema: 'public', table: 'assignment_safety_alerts', filter: `organization_id=eq.${organizationId}` }, () => void load()).subscribe()
  ...
  const updateAssignment = async (patch: Partial<Pick<Assignment, 'status' | 'priority' | 'assigned_team_id' | 'assigned_user_id' | 'sequence_number'>>) => {
    ...
    try {
      ...
      setAssignments(items => items.map(item => item.id === data.id ? data as Assignment : item)); setMessage('Assignment updated and history preserved.')
  ...
  const acknowledge = async (alertId: string) => {
    ...
    const { error: updateError } = await supabase.from('assignment_safety_alerts').update({ status: 'acknowledged', acknowledged_by: authData.user?.id, acknowledged_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', alertId).eq('organization_id', organizationId)
  ...
  return <div className="assignment-operations">
    ...
    {error && <div className="rw-context-alert" role="alert">{error}</div>}{message && <div className="assignment-message" role="status">{message}</div>}
