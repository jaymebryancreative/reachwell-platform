...
import { archivePerson, createPerson, listPeople, listPersonTeamMemberships, type PersonRecord } from '../../lib/reachwellApi'
...
export function PeopleWorkspace() {
  ...
  const load = async () => {
    if (!organizationId) {
      ...
      setSelectedId(null)
      setLoading(false)
      setError(contextLoading ? null : 'Sign in with an active ReachWell organization membership to view people.')
      return
    }
    setLoading(true)
    setError(null)
    try {
      ...
      const next = await Promise.all(records.map(async person => {
        const memberships = await listPersonTeamMemberships(organizationId, person.id)
        return { ...person, teams: memberships.map(membership => membership.team?.name).filter(Boolean) as string[] }
      }))
      setPeople(next)
  ...
  return <div className="rw-people">
    ...
    <div className="rw-people-layout">
      <aside className="rw-people-list-panel"><label className="rw-search"><Search size={17}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search people" /></label><div className="rw-person-list">{(loading || contextLoading) && <div className="rw-roster-empty">Loading organization people…</div>}{!loading && !contextLoading && filtered.map(person => <button key={person.id} onClick={() => setSelectedId(person.id)} className={person.id === selected?.id ? 'is-selected' : ''}><span className="rw-person-avatar">{person.first_name[0]}{person.last_name[0]}</span><span><strong>{person.preferred_name || `${person.first_name} ${person.last_name}`}</strong><small>{person.teams.length ? person.teams.join(' · ') : 'No teams assigned'}</small></span></button>)}{!loading && !contextLoading && organizationId && !filtered.length && <div className="rw-roster-empty">No active people match this search.</div>}</div></aside>
      {selected ? <section className="rw-person-detail"><div className="rw-person-detail-head"><div><span className="rw-eyebrow">Active profile</span><h2>{selected.preferred_name || `${selected.first_name} ${selected.last_name}`}</h2><p>{selected.email || 'No email yet'}{selected.phone ? ` · ${selected.phone}` : ''}</p></div><button className="rw-secondary-button" onClick={() => void archiveSelected()}><Archive size={17}/> Archive</button></div><div className="rw-person-summary"><div><strong>{selected.teams.length}</strong><span>Teams</span></div><div><strong>0</strong><span>Events served</span></div><div><strong>0</strong><span>Follow-ups</span></div></div><div className="rw-profile-section"><h3>Team connections</h3>{selected.teams.length ? <div className="rw-tag-row">{selected.teams.map(team => <span key={team} className="rw-tag">{team}</span>)}</div> : <p className="rw-muted">No team assignments yet. Assign this person to an organization team to connect their field work.</p>}</div></section> : <section className="rw-person-detail"><div className="rw-roster-empty">{organizationId ? 'Add a person to begin.' : 'Organization context is required.'}</div></section>}
    </div>
