...
type Signal = { id: string; title: string; detail: string; kind: 'need' | 'prayer' | 'followup'; personId?: string | null; householdId?: string | null }

export function RelationshipHealthWorkspace({ onOpenTimeline }: { onOpenTimeline?: (target: { personId?: string; householdId?: string }) => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSignals([
        ...(needs.data ?? []).map(row => ({ id: row.id, title: row.title, detail: row.description || 'Need surfaced through outreach.', kind: 'need' as const, personId: row.person_id, householdId: row.household_id })),
        ...(prayers.data ?? []).map(row => ({ id: row.id, title: 'Prayer request', detail: row.request_text, kind: 'prayer' as const, personId: row.person_id, householdId: row.household_id })),
        ...(followUps.data ?? []).map(row => ({ id: row.id, title: row.title, detail: row.description || (row.due_at ? `Due ${new Date(row.due_at).toLocaleDateString()}` : 'Follow-up still open.'), kind: 'followup' as const, personId: row.person_id, householdId: row.household_id })),
      ].slice(0, 24))
    } catch (err) {
  ...
  const openSignal = (signal: Signal) => {
    if (!onOpenTimeline || (!signal.personId && !signal.householdId)) return
    onOpenTimeline(signal.personId ? { personId: signal.personId } : { householdId: signal.householdId as string })
  }
  ...
  return <div className="relationship-workspace">
    ...
    <section className="relationship-card"><div className="relationship-card-heading"><div><span className="rw-eyebrow">CONNECTED SIGNALS</span><h2>Where another touchpoint may matter</h2></div><HeartHandshake size={22}/></div>
      {loading ? <div className="relationship-empty">Loading relationship signals…</div> : !signals.length ? <div className="relationship-empty"><HeartHandshake size={26}/><strong>No open relationship signals</strong><span>Needs, prayer requests, and follow-ups will appear here as outreach surfaces them.</span></div> : <div className="relationship-signal-grid">{signals.map(signal => <article key={`${signal.kind}-${signal.id}`} className="relationship-signal" role={signal.personId || signal.householdId ? 'button' : undefined} tabIndex={signal.personId || signal.householdId ? 0 : undefined} onClick={() => openSignal(signal)} onKeyDown={event => { if ((event.key === 'Enter' || event.key === ' ') && (signal.personId || signal.householdId)) openSignal(signal) }}><div className={`relationship-signal-icon ${signal.kind}`}>{signal.kind === 'need' ? <AlertCircle size={17}/> : signal.kind === 'prayer' ? <Moon size={17}/> : <CalendarClock size={17}/>}</div><div><strong>{signal.title}</strong><p>{signal.detail}</p><small>{signal.kind === 'need' ? 'Need' : signal.kind === 'prayer' ? 'Prayer' : 'Follow-up'}{signal.personId || signal.householdId ? ' · View connected history' : ''}</small></div></article>)}</div>}
    </section>
