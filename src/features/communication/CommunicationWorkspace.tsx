...
import { MessageCircle, Plus, RefreshCw, Search, Send, Archive, CheckCheck, Pencil, Trash2, Users, Heart, HandHeart, Megaphone, CalendarDays, Shield, Sparkles, House, Star, Palette, Settings2 } from 'lucide-react'
...
type Preset = { name: string; description: string; accent: Accent; icon: IconKey }
const PRESETS: Preset[] = [
  { name: 'Outreach Team', description: 'Field outreach and community connection', accent: 'violet', icon: 'megaphone' },
  { name: 'Welcome Team', description: 'Guests, hospitality, and first connections', accent: 'blue', icon: 'hand-heart' },
  { name: 'Prayer Team', description: 'Prayer requests and encouragement', accent: 'rose', icon: 'heart' },
  { name: 'Care Team', description: 'Care, follow-up, and practical support', accent: 'teal', icon: 'heart' },
  { name: 'Leadership Team', description: 'Leadership coordination and decisions', accent: 'indigo', icon: 'shield' },
  { name: 'Events Team', description: 'Event planning and event-day coordination', accent: 'amber', icon: 'calendar' },
  { name: 'Communications Team', description: 'Announcements, content, and messaging', accent: 'orange', icon: 'megaphone' },
  { name: 'Volunteer Team', description: 'Volunteer coordination and updates', accent: 'green', icon: 'users' },
]
...
export function CommunicationWorkspace() {
  ...
  const loadMessages = async () => {
    ...
    if (user && next.length) await supabase.from('communication_message_reads').upsert(next.map(message => ({ message_id: message.id, user_id: user.id, read_at: new Date().toISOString() })), { onConflict: 'message_id,user_id' })
  }
  ...
  const applyPreset = (preset: Preset) => { setNewName(preset.name); setNewDescription(preset.description); setNewAccent(preset.accent); setNewIcon(preset.icon) }

  const createChannel = async () => {
    if (!organizationId || !user || !newName.trim()) return
    setError('')
    const { data, error: createError } = await supabase.from('communication_channels').insert({ organization_id: organizationId, name: newName.trim(), description: newDescription.trim() || null, channel_type: 'organization', created_by: user.id, is_private: false, accent_color: newAccent, icon_key: newIcon }).select('id,name,description,channel_type,team_id,event_id,assignment_id,is_private,created_by,archived_at,accent_color,icon_key').single()
    if (createError) { setError(createError.message); return }
    setNewName(''); setNewDescription(''); setNewAccent('violet'); setNewIcon('message-circle'); setShowCreate(false); await loadChannels(); if (data?.id) setSelectedId(data.id)
  }
  ...
  const saveCustomization = async () => {
    ...
    setShowCustomize(false); await loadChannels()
  }

  const openCustomization = () => { if (!selected) return; setNewName(selected.name); setNewDescription(selected.description ?? ''); setNewAccent(selected.accent_color); setNewIcon(selected.icon_key); setShowCustomize(true) }
  ...
  return <div className="communication-workspace">
    <header className="money-trail-heading"><div><span className="rw-eyebrow">REACHWELL COMMUNICATION</span><h1>Communication</h1><p>Make team conversations feel like your organization.</p></div><div style={{ display: 'flex', gap: 8 }}><button className="rw-secondary-button" onClick={() => void loadChannels()} disabled={loading}><RefreshCw size={16}/> {loading ? 'Refreshing…' : 'Refresh'}</button>{canManage(organizationRole) && <button className="rw-primary-button" onClick={() => setShowCreate(value => !value)}><Plus size={16}/> New channel</button>}</div></header>
    {error && <div className="rw-context-alert" role="alert">{error}</div>}
    {showCreate && canManage(organizationRole) && <div className="rw-card communication-customize-card"><div><span className="rw-eyebrow">CHANNEL SETUP</span><h2>Start with a ReachWell preset</h2><p>Use a native team identity, then personalize the name, color, and icon.</p></div><div className="communication-preset-grid">{PRESETS.map(preset => { const PresetIcon = iconFor(preset.icon); return <button key={preset.name} className="communication-preset" onClick={() => applyPreset(preset)}><span className={`communication-color-dot ${preset.accent}`}><PresetIcon size={15}/></span><span><strong>{preset.name}</strong><small>{preset.description}</small></span></button> })}</div><div className="communication-customize-grid"><label>Team / channel name<input className="rw-input" value={newName} onChange={event => setNewName(event.target.value)} placeholder="Channel name" /></label><label>Description<input className="rw-input" value={newDescription} onChange={event => setNewDescription(event.target.value)} placeholder="What is this chat for?" /></label><label>Color<select className="rw-input" value={newAccent} onChange={event => setNewAccent(event.target.value as Accent)}>{ACCENTS.map(accent => <option key={accent} value={accent}>{accent[0].toUpperCase() + accent.slice(1)}</option>)}</select></label><label>Icon<select className="rw-input" value={newIcon} onChange={event => setNewIcon(event.target.value as IconKey)}>{ICONS.map(icon => <option key={icon} value={icon}>{icon.replaceAll('-', ' ')}</option>)}</select></label></div><button className="rw-primary-button" onClick={() => void createChannel()} disabled={!newName.trim()}>Create channel</button></div>}
    <div className="communication-layout">
      <aside className="communication-channels"><div className="ops-search"><Search size={16}/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search channels and messages" aria-label="Search communication" /></div>{filteredChannels.map(channel => { const ChannelIcon = iconFor(channel.icon_key); return <button key={channel.id} className={channel.id === selectedId ? 'selected' : ''} style={{ '--channel-accent': `var(--rw-${channel.accent_color}, var(--rw-violet,#6f52ff))` } as React.CSSProperties} onClick={() => setSelectedId(channel.id)}><span className={`communication-channel-icon ${channel.accent_color}`}><ChannelIcon size={16}/></span><span>{channel.name}<small>{channel.team_id ? 'Team' : channel.event_id ? 'Event' : channel.assignment_id ? 'Mission assignment' : 'Organization'}</small></span></button> })}{!filteredChannels.length && <div className="rw-empty-state">No channels match your search.</div>}</aside>
      <section className="communication-thread">{selected ? <><div className={`communication-thread-head ${selected.accent_color}`}><div className="communication-thread-title"><span className={`communication-channel-icon large ${selected.accent_color}`}><SelectedIcon size={18}/></span><div><strong>{selected.name}</strong><small>{selected.description || 'Organization conversation'}</small></div></div><div style={{ display: 'flex', gap: 8 }}>{canManage(organizationRole) && <button className="rw-secondary-button" onClick={openCustomization}><Settings2 size={15}/> Customize</button>}{canManage(organizationRole) && <button className="rw-secondary-button" onClick={() => void archiveChannel()}><Archive size={15}/> Archive</button>}</div></div>{showCustomize && canManage(organizationRole) && <div className="communication-customize-inline"><div><strong>Personalize this team chat</strong><small>Admins and owners can change the shared team identity.</small></div><div className="communication-customize-grid"><label>Team / channel name<input className="rw-input" value={newName} onChange={event => setNewName(event.target.value)} /></label><label>Description<input className="rw-input" value={newDescription} onChange={event => setNewDescription(event.target.value)} /></label><label>Color<select className="rw-input" value={newAccent} onChange={event => setNewAccent(event.target.value as Accent)}>{ACCENTS.map(accent => <option key={accent} value={accent}>{accent[0].toUpperCase() + accent.slice(1)}</option>)}</select></label><label>Icon<select className="rw-input" value={newIcon} onChange={event => setNewIcon(event.target.value as IconKey)}>{ICONS.map(icon => <option key={icon} value={icon}>{icon.replaceAll('-', ' ')}</option>)}</select></label></div><div style={{ display:'flex', gap:8 }}><button className="rw-primary-button" onClick={() => void saveCustomization()}>Save changes</button><button className="rw-secondary-button" onClick={() => setShowCustomize(false)}>Cancel</button></div></div>}
