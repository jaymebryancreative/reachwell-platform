...
type Member = { id: string; user_id: string; role: string; status: string; joined_at: string | null }
...
export function AdminWorkspace() {
  ...
  return <div className="admin-workspace">
    ...
    <section className="admin-card"><div className="admin-card-heading"><div><span className="rw-eyebrow">MEMBERS</span><h2>Current organization access</h2></div><UserRoundCog size={21}/></div><div className="admin-members">{members.map(member => <div className="admin-member" key={member.id}><div><strong>{member.role.replace('_',' ')}</strong><small>User {member.user_id.slice(0, 12)} · {member.status}</small></div><select value={member.role} disabled={saving || member.role === 'owner'} onChange={e => void changeRole(member.user_id, e.target.value)} aria-label={`Change role for ${member.user_id}`}>{member.role === 'owner' && <option value="owner">owner</option>}{roles.map(value => <option key={value} value={value}>{value.replace('_',' ')}</option>)}</select></div>)}{!members.length && <div className="rw-empty-state">No organization members yet.</div>}</div></section>
