  ...
  const load = async () => {
    ...
    try {
      const [memberResult, invitationResult] = await Promise.all([
        supabase.from('organization_members').select('id,user_id,role,status,joined_at').eq('organization_id', organizationId).order('role').order('joined_at'),
      ...
      const rawMembers = (memberResult.data ?? []) as Member[]
      const userIds = rawMembers.map(member => member.user_id)
      const profileResult = userIds.length ? await supabase.from('profiles').select('id,full_name,first_name,last_name,phone').in('id', userIds) : { data: [], error: null }
      if (profileResult.error) throw profileResult.error
      const profiles = new Map((profileResult.data ?? []).map(profile => [profile.id, profile]))
      setMembers(rawMembers.map(member => ({ ...member, profile: profiles.get(member.user_id) ?? null })))
