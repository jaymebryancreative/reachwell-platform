...
import { listAssignmentActivity, listAssignmentHistory, type AssignmentActivityRecord, type AssignmentRecord } from '../../lib/reachwellApi'
import { useReachWellContext } from '../../lib/reachwellContext'
import './activity.css'

const labels: Record<string, string> = { assignment_created: 'Assignment created', assignment_status_changed: 'Assignment status changed', assignment_updated: 'Assignment updated', objective_created: 'Objective added', objective_completed: 'Objective completed', objective_updated: 'Objective updated', visit_started: 'Visit started', visit_finished: 'Visit finished', visit_updated: 'Visit updated', note_added: 'Note added', need_recorded: 'Need recorded', prayer_recorded: 'Prayer request recorded', follow_up_created: 'Follow-up created', follow_up_completed: 'Follow-up completed', follow_up_updated: 'Follow-up updated' }
function iconFor(type: string) { if (type.includes('visit')) return MapPin; if (type.includes('objective') || type.includes('complete')) return CheckCircle2; if (type.includes('need')) return CircleAlert; if (type.includes('prayer')) return Heart; if (type.includes('note')) return FileText; return Activity }
...
export function ActivityWorkspace() {
  ...
  useEffect(() => {
    ...
    void listAssignmentHistory(organizationId, user.id).then(items => { setAssignments(items); setSelectedId(current => current || items[0]?.id || '') }).catch(err => setError(err instanceof Error ? err.message : 'Unable to load assignment history.')).finally(() => setLoading(false))
  ...
  return <div className="activity-workspace">
    ...
    <div className="activity-layout">
      <section className="activity-panel"><div className="activity-panel-head"><strong>Assignment history</strong><span>{assignments.length}</span></div>{loading && !assignments.length && <p className="activity-muted">Loading…</p>}{!loading && !assignments.length && <p className="activity-muted">No assignments are available in your history.</p>}{assignments.map(item => <button key={item.id} className={item.id === selectedId ? 'activity-assignment selected' : 'activity-assignment'} onClick={() => setSelectedId(item.id)}><span>{item.address_label || item.title}</span><small>{item.status.replaceAll('_', ' ')}</small></button>)}</section>
